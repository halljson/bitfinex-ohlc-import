...
def get_symbols():
    ...
    pair_df = df[df[0].str.startswith('t')].copy()
    pair_df[0] = pair_df[0].apply(lambda x: x[1:].lower())
    symbols = pair_df[0].tolist()
    symbols = [sym for sym in symbols if (sym.endswith('usd') or sym.endswith('ust'))]
    symbols.remove('exousd') # fails for some reason
    symbols.remove('testbtc:testusd') # wtf is this
    return symbols
